...
export function ProductCategories() {
  ...
  const { data } = api.product.getProductCategories.useQuery();
  return (
              ...
              data.map((category, index) => {
                return (
                  <div key={index} className="relative w-[250px] space-y-3">
                    <Image
                      src={category.image ?? "https://picsum.photos/id/288/450"}
                      alt={category.name}
                      width={250}
                      height={250}
                      className={cn(
                    ...
                    <h2
                      className={cn(
                        "absolute left-1/2 top-1/2 w-max -translate-x-1/2 -translate-y-1/2 transform rounded-sm bg-gradient-to-b from-red-200 to-gray-100 p-2 text-lg font-bold uppercase "
                      )}
                    >
                      {category.name}
                    </h2>
